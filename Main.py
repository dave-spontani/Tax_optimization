...
with tab1:
    ...
    with st.form("qform"):
        ...
        pillar2_contrib = st.number_input("Mandatory 2nd pillar contributions (CHF)", min_value=0.0, value=6000.0)
        pillar3a_current = st.number_input("Voluntary Pillar 3a contributions already this year (CHF)", min_value=0.0, value=0.0)
        private_insurance_premiums = st.number_input("Private insurance premiums (annual) (CHF)", min_value=0.0, value=300.0)
    ...
    if not submitted:
        ...
        if pillar3a_current > pillar3a_cap:
            st.warning(f"Pillar 3a you entered ({pillar3a_current:.2f} CHF) exceeds the estimated cap ({pillar3a_cap:.2f} CHF). For estimates we'll only use the cap amount.")
        ...
        deductions["Private insurance premiums"] = round(private_insurance_premiums, 2)
